import { c as create_ssr_component, l as add_attribute, e as escape, i as createEventDispatcher, o as onDestroy, v as validate_component } from "./ssr.js";
...
const PitchDetector_1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  function detectPitch() {
    ...
    if (pitch && clarity > 0.6 && currentVolume > 10 && isValidVocalRange) {
      ...
      if (correctedFreq !== pitch && Math.abs(correctedFreq - pitch) > 5 && trainingPhase === "guiding") {
        const ratio = pitch / correctedFreq;
        const correctionType = ratio > 1.8 && ratio < 2.2 ? "2x" : ratio > 2.8 && ratio < 3.2 ? "3x" : ratio > 3.8 && ratio < 4.2 ? "4x" : ratio > 0.45 && ratio < 0.55 ? "1/2x" : "other";
        const noteOrig = frequencyToNote(pitch);
        const noteCorrected = frequencyToNote(correctedFreq);
        const logMessage = `🔧 [Harmonic] ${pitch.toFixed(0)}Hz(${noteOrig}) → ${correctedFreq.toFixed(0)}Hz(${noteCorrected}) [${correctionType}補正]`;
        const currentTime = Date.now();
