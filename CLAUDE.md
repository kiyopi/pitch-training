...
- **方式**: 基音再生→ユーザー歌唱→リアルタイム音程検出→正解判定
- **対象音階**: ドレミファソラシド（8音階）の相対音程
- **技術**: Web Audio API + Pitchy（McLeod Pitch Method）による高精度音程検出

### 実装されているトレーニングモード
1. **full-scale-training.html**: 高機能版（ランダム基音、ノイズリダクション、デバッグ機能）
2. **simple-pitch-training.html**: シンプル版（基本機能に特化、v2.0.0-simple-clean）
3. **minimal-pitch-training.html**: 最小構成版（実験用）
